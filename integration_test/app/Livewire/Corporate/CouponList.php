...
class CouponList extends Component
{
    ...
    public function mount()
    {
        ...
        $this->prefixes = CouponCode::whereNotNull('prefix')->where('corporate_id', $corporate->id)->where('status', 1)->groupBy('prefix')->pluck('prefix', 'prefix');
        $this->values = CouponCode::whereNotNull('value')->where('corporate_id', $corporate->id)->where('status', 1)->groupBy('value')->orderBy('value', 'asc')->pluck('value', 'value');
    }

    public function render()
    {
        $corporate = Auth::guard('corporate')->user();
        $query = CouponCode::where('corporate_id', $corporate->id)->where('status', 1)->orderByDesc('created_at');
